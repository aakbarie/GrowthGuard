...
saudi_wa60 <- function(sex=c("m", "f")) {
  ...
  suppressMessages(wa_data <- read_csv("./R/Posts/Growth/data/wg_60.csv"))
  ...
  plot(lowess(wa_data$Agemos, wa_data$P3, f=.01), 
       type="l", ylim=c(3, 25), lwd=2, axes=F, xlab = "", ylab = "")
  title(main="The Growth Charts for Children and Adolescents", 
        xlab="Age (Months)", ylab="Weight (kg)")
  mtext(
    paste0("Weight-for-age percentiles: ",gender,", birth to 60 months"),
    cex=.95, font=3, col="red")
  
  # y axis left
  axis(side=2, at=seq(3, 25, 0.25), labels=FALSE, col.ticks=color)
  axis(side=2, at=seq(3, 25,  1), labels=seq(3, 25,  1), lwd.ticks=2)
  
  # y axis right
  axis(side=4, at=seq(3, 25,  0.5), labels=FALSE, col.ticks=color)
  axis(side=4, at=seq(3, 25,  1), labels=seq(3, 25,  1), lwd.ticks=2)
  ...
  abline(h=seq(3, 25, 0.2), col=color)
