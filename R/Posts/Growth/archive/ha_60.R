...
saudi_wa60 <- function(sex=c("m", "f")) {
  getwd()
  library(tidyverse)
  
  suppressMessages(wa_data <- read_csv("./R/Posts/Growth/data/hg_60.csv"))
  print("what")
  ...
  plot(lowess(wa_data$Agemos, wa_data$P3, f=.01), 
       type="l", ylim=c(40, 120), lwd=2, axes=F, xlab = "", ylab = "")
  title(main="The Growth Charts for Children and Adolescents", 
        xlab="Age (Months)", ylab="Length (cm)")
  mtext(
    paste0("Height-for-age percentiles: ",gender,", birth to 60 months"),
    cex=.95, font=3, col="red")
  
  # y axis left
  axis(side=2, at=setdiff(seq(40, 120, 1), seq(40, 120, 5)), labels=FALSE, col.ticks=color)
  axis(side=2, at=seq(40, 120,  5), labels=seq(40, 120,  5), lwd.ticks=2)
  
  # y axis right
  axis(side=4, at=setdiff(seq(40, 120, 1), seq(40, 120, 5)), labels=FALSE, col.ticks=color)
  axis(side=4, at=seq(40, 120,  5), labels=seq(40, 120,  5), lwd.ticks=2)
  
  # x axis down
  axis(side=1, at=seq(0, 60, 0.5), labels=FALSE, col.ticks=color)
  axis(side=1, at=seq(0, 60, 1), labels=seq(0, 60, 1), lwd.ticks=2)
  ...
  abline(v=seq(0, 60, 1),   col=color)
  abline(h=seq(40, 120, 1), col=color)
